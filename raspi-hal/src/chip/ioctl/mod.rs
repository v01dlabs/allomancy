// Copyright (c) 2024 Orual, based on work by the rust-gpio-cdev Project Developers.
// 
// Updated to use the v2 ABI
//
...
use std::slice;
use std::sync::Arc;
use crate::chip::ioctl::ffi::gpio_v2_line_attr_id as line_attr_id;
...
/// Iterator over GPIO Lines for a given chip.
#[derive(Debug)]
pub struct LineIteratorV2 {
    chip: Arc<InnerChip>,
    idx: u32,
}

impl Iterator for LineIteratorV2 {
    type Item = LineV2;

    fn next(&mut self) -> Option<LineV2> {
        if self.idx < self.chip.lines {
            let idx = self.idx;
            self.idx += 1;
            // Since we checked the index, we know this will be Ok
            Some(LineV2::new(self.chip.clone(), idx).unwrap())
        } else {
            None
        }
    }
...
bitflags! {
    ...
    /// Maps to kernel [`GPIOLINE_FLAG_*`] flags.
    ///
    /// [`GPIO_V2_LINE_FLAG_*`]: https://elixir.bootlin.com/linux/v4.9.127/source/include/uapi/linux/gpio.h#L29
    #[derive(Debug, Clone, Copy)]
    pub struct LineFlagsV2: u64 {
        ...
        const EVENT_CLOCK_RT = (1 << 11);
        const EVENT_CLOCK_HTE = (1 << 12);
    }
}

pub enum LineAttributeV2 {
    Flags(LineFlagsV2),
    OutputValues(u64),
    DebouncePeriodUs(u32),
}

pub fn offsets_to_mask(offsets: &[u32]) -> u64 {
    offsets.iter().fold(0, |acc, &x| acc | (1 << x))
}

pub struct LineCfgAttributeV2 {
    pub attr: LineAttributeV2,
    pub mask: u64,
}

impl LineCfgAttributeV2 {
    pub fn new(attr: LineAttributeV2, offset: u32) -> Self {
        let mask = 1 << offset;
        Self { attr, mask }
    }

    pub fn new_multi(attr: LineAttributeV2, offsets: &[u32]) -> Self {
        Self { attr, mask: offsets_to_mask(offsets) }
    }

    /// Get the offset of the single bit set in the mask, or the lowest bit if multiple bits are set
    pub fn offset(&self) -> u32 {
        self.mask.trailing_zeros()
    }

    pub fn offsets(&self) -> Vec<u32> {
        let mut offsets = vec![0u32; self.mask.count_ones() as usize];
        for i in 0..64 {
            if self.mask & (1 << i) != 0 {
...
pub struct LineConfiguration {
    pub global_flags: LineFlagsV2,
    pub attributes: Vec<LineCfgAttributeV2>,
}

impl LineConfiguration{
    pub fn create(offset: u32, flags: LineFlagsV2, default_value: Option<bool>, debounce_period_us: Option<u32>) -> Self {
        let mut attributes = Vec::new();
        if let Some(default_value) = default_value {
            attributes.push(LineCfgAttributeV2{attr: LineAttributeV2::OutputValues(if default_value {1} else {0}), mask: 1 << offset});
        }
        if let Some(debounce_period_us) = debounce_period_us {
            attributes.push(LineCfgAttributeV2{attr: LineAttributeV2::DebouncePeriodUs(debounce_period_us), mask: 1 << offset});
        }
        Self { global_flags: flags, attributes }
    }

    pub fn create_multi<const LEN:usize>(flags: &[(u32, LineFlagsV2); LEN], default_values: &[Option<bool>; LEN], debounce_period_us: Option<u32>) -> Self {
        let global_flags = flags
            .iter().fold(LineFlagsV2::all(), |acc, &x| acc & x.1);
        let set_pins = flags.iter().fold(0u64, |acc, &(x, _f)| acc | (1 << x));
        let default_values = default_values
            .iter().enumerate()
            .fold(0u64, |acc, (index, &value)|{ 
                if Some(true) == value {
                    acc | (1 << flags[index].0)
                } else {
                    acc
                }});
        let mut attributes = vec![LineCfgAttributeV2{ attr: LineAttributeV2::OutputValues(default_values), mask: set_pins}];
        if let Some(debounce_period_us) = debounce_period_us {
            attributes.push(LineCfgAttributeV2{attr: LineAttributeV2::DebouncePeriodUs(debounce_period_us), mask: set_pins});
        }
        flags.iter().for_each(|&(offset, flag)| {
            if !(global_flags.contains(flag)) {
                attributes.push(LineCfgAttributeV2::new(LineAttributeV2::Flags(flag.symmetric_difference(global_flags)), offset));
            }
        });
        Self { global_flags, attributes }
    }
}
/// In or Out
#[derive(Debug, Clone, Copy, PartialEq)]
...
impl LineV2 {
    ...
    /// Get info about the line from the kernel.
    pub fn info(&self) -> Result<LineInfoV2> {
        let mut line_info = ffi::gpio_v2_line_info {
            name: [0; ffi::GPIO_MAX_NAME_SIZE],
            consumer: [0; ffi::GPIO_MAX_NAME_SIZE],
            line_offset: self.offset,
            num_attrs: 0,
            flags: 0,
            attrs: core::array::from_fn(|_|ffi::gpio_v2_line_attribute {
                id: 0,
                padding: 0,
        ...
        ffi::gpio_v2_get_lineinfo_ioctl(self.chip.file.as_raw_fd(), &mut line_info)?;

        Ok(LineInfoV2 {
            line: self.clone(),
            flags: LineFlagsV2::from_bits_truncate(line_info.flags),
            name: unsafe { cstrbuf_to_string(&line_info.name[..]) },
            consumer: unsafe { cstrbuf_to_string(&line_info.consumer[..]) },
        })
    }
    ...
    pub fn request(
        ...
        // prepare the request; the kernel consumes some of these values and will
        // set the fd for us.
        let mut request = ffi::gpio_v2_line_request {
            lineoffsets: unsafe { mem::zeroed() },
            consumer: unsafe { mem::zeroed() },
            num_lines: 1,
            config: ffi::gpio_v2_line_config { 
                flags: flags.bits(), 
                num_attrs: 0, 
                padding: [0; 5], 
                attrs: core::array::from_fn(|_| ffi::gpio_v2_line_config_attribute {
                    attr:ffi::gpio_v2_line_attribute {
                        id: line_attr_id::GPIO_V2_LINE_ATTR_ID_OUTPUT_VALUES as u32,
                        padding: 0,
                        union_flags_values_debounce: 0,
                    },
                    mask: 0,
        ...
        ffi::gpio_v2_get_line_ioctl(self.chip.file.as_raw_fd(), &mut request)?;
        Ok(LineAccessV2 {
            line: self.clone(),
            flags,
            file: unsafe { File::from_raw_fd(request.fd) },
...
impl LineAccessV2 {
    ...
    /// Get the Line information associated with this handle.
    pub fn line(&self) -> &LineV2 {
        &self.line
    }

    /// Get the flags with which this handle was created
    pub fn flags(&self) -> LineFlagsV2 {
        self.flags.clone()
    }
}


impl AsRawFd for LineAccessV2 {
    /// Gets the raw file descriptor for the `LineHandle`.
    fn as_raw_fd(&self) -> RawFd {
        self.file.as_raw_fd()
    }
